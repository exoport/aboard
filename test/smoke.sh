#!/usr/bin/env sh
# Headless smoke test: mounts every view against the real board.json and checks
...
if [ -x ./board ]; then
  ...
  # board.html is included: it is the last place the "one way to make a button"
...
PNG=$(mktemp /tmp/board-smoke-XXXXXX.png)
node -e "
