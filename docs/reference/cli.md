...
```
aboard shot <tab>... [flags]
```

Take a picture of a tab as the human sees it, and print where it went. Then
READ the picture: `aboard apply` exits 0 for a tree that draws an empty box, and
...
What it knows so you do not have to:
  ...
  mount receipts        none are posted: `aboard rendered` and
                        `aboard wait --for "rendered <id>"` go on meaning a person
                        had the tab open
