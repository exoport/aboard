# Why four guarantees are server-enforced

Four things an agent cannot do to a board, no matter what it writes:
...
The rule of thumb the four share: *if forgetting it destroys something that cannot be
reconstructed, it is not a convention.* The human's attention is the scarcest resource in
this system, and three of the four guarantees are about not spending it silently.
...
interface; it enforces nothing. The four guarantees are the opposite bargain — invisible
