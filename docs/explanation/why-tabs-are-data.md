...
marks-hidden, chat drafts and each tab's scroll position: those belong to one viewer's
browser. Putting them in shared state means one person's scrolling is a write everyone
else's board reacts to.

**A tab carries a `note` — the agent's brief statement of what it is FOR, which the
human may edit.** Read it before acting
...
What they want DONE about the tab is a different field — `requests` — and the split is
deliberate: a purpose rewritten into a to-do stops being a purpose, and a to-do living in
the purpose strip has nowhere to record that it was dealt with.
