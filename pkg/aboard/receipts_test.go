...
func TestARenderedReceiptComesBackOutOfTheSidecar(t *testing.T) {
	...
	if rec := postReceipt(t, srv, `{"tab":"bb1","type":"ui","mount":true,"controls":["fit","relayout"],
...
func TestReceiptsAccumulatePressesAndReplaceMarkers(t *testing.T) {
	srv := testServer(t, htmlTabBoard)
	postReceipt(t, srv, `{"tab":"bb1","type":"ui","mount":true,"unknown":["sparkline"],"fired":{"fit":1}}`)
	postReceipt(t, srv, `{"tab":"bb1","type":"ui","mount":true,"fired":{"fit":1,"relayout":3}}`)
...
func TestAReceiptNeverTouchesTheBoardDocument(t *testing.T) {
	...
	postReceipt(t, srv, `{"tab":"bb1","type":"ui","controls":["fit"]}`)
...
func TestRenderedPrintsWhatItIsNotEvidenceOf(t *testing.T) {
	got := RenderedHuman("bb1", nil)
	if !strings.Contains(got, "no mount receipt for bb1") {
...
func TestRenderedReleasesAWaitingSession(t *testing.T) {
	srv := testServer(t, htmlTabBoard)
	pred, err := parsePredicate("rendered bb1")
	if err != nil {
		t.Fatal(err)
	}
	w := srv.waits.add("agent-1", "rendered bb1", "", pred, time.Minute)

	// The wrong tab must not release it: a predicate that fires on anything is a
	// predicate that tells the caller nothing.
	postReceipt(t, srv, `{"tab":"bb2","type":"ui"}`)
	select {
	case ev := <-w.ch:
	...
	postReceipt(t, srv, `{"tab":"bb1","type":"ui"}`)
	select {
	case ev := <-w.ch:
		if ev.Event != "rendered" || ev.Note != "bb1" {
			t.Errorf("unexpected release event: %+v", ev)
		}
	case <-time.After(time.Second):
		t.Fatal("a receipt for bb1 did not release a session waiting on it")
	}
}
...
func TestTheRenderedPredicateIsInTheVocabulary(t *testing.T) {
	...
	if _, err := parsePredicate("nonsense bb1"); err == nil || !strings.Contains(err.Error(), "rendered <id>") {
	...
	if pred.matches([]byte(`{}`), JournalEntry{By: "human", Tabs: []string{"bb1"}}) {
