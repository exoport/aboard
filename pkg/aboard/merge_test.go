...
const mergeBoard = `{
  ...
  "nextId": 9,
  "tabs": [
    {"id": "bb1", "name": "Mine", "type": "notes", "state": {"text": "base"}},
    {"id": "bb2", "name": "Theirs", "type": "notes", "state": {"text": "base"}}
  ]
}`
...
// to bb2 refuses a write to bb1 — and `apply` used to discard the agent's whole
...
func TestApplyMergesAConflictOnAnotherTab(t *testing.T) {
	...
	setTabText(t, theirs, "bb2", "theirs")
	...
	if !strings.Contains(errOut, "bb2") {
	...
	if got := tabText(t, root, "bb2"); got != "theirs" {
		t.Errorf("bb2 = %q, want the other session's write kept — our document still held its old state", got)
	}
}

// The browser's rule, kept: a genuine same-tab collision is never merged
// silently. Both sides changed bb1, and picking a winner is not a decision a
// retry gets to make.
func TestApplyRefusesASameTabCollisionByName(t *testing.T) {
	...
	theirs := readBoard(t, root)
	setTabText(t, theirs, "bb1", "theirs")
	if _, _, err := applyDoc(t, root, theirs, "agent-2"); err != nil {
		t.Fatal(err)
	}
	...
	if !strings.Contains(err.Error(), "bb1") {
		t.Errorf("the refusal must NAME the tab, got %q", err)
	}
	if got := tabText(t, root, "bb1"); got != "theirs" {
		t.Errorf("bb1 = %q, want the other session's write left alone", got)
	}
}
...
func TestMergeKeepsATabCreatedSinceOurBase(t *testing.T) {
	...
	theirs["tabs"] = append(theirTabs, map[string]any{
		"id": "bb9", "name": "New", "type": "notes", "state": map[string]any{"text": "new"},
	})
	if _, _, err := applyDoc(t, root, theirs, "agent-2"); err != nil {
		t.Fatal(err)
	}
	...
	if got := tabText(t, root, "bb9"); got != "new" {
		t.Errorf("bb9 = %q, want the tab the other session created still there", got)
	}
...
func TestMergeRetriesOnlyOnce(t *testing.T) {
	...
	spy.once("/journal", func() { writeInProcess(t, srv, "bb2", "third", "agent-3") })
...
func TestForceStillWritesWithoutCompareAndSet(t *testing.T) {
	...
	if got := tabText(t, root, "bb1"); got != "mine" {
		t.Errorf("bb1 = %q, want the forced write to have landed", got)
	}
}

// The seam between two features that were built in parallel: `apply --label`
// (bb371) puts the caller's reason on the journal entry, and the 409 merge
// (bb366) posts a document rebuilt from the board's own. A label written onto
...
func TestAMergedWriteKeepsTheLabelOfTheWriteItRedoes(t *testing.T) {
	...
	var out, errOut bytes.Buffer
	options := ApplyOptions{By: "agent-1", Label: "restating bb1 after review"}
	...
	if last.Label != "restating bb1 after review" {
