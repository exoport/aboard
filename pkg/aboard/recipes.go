...
func readRecipeTier(tier recipeTier) ([]Recipe, error) {
	...
	return readRecipeFS(os.DirFS(tier.dir), ".", tier.scope,
		func(_, name string) string { return filepath.Join(tier.dir, name) })
}
...
func readRecipeFS(fsys fs.FS, dir, scope string, join func(string, string) string) ([]Recipe, error) {
	...
	for _, e := range entries {
		...
		if err != nil {
			return nil, fmt.Errorf("reading %s: %w", join(dir, e.Name()), err)
		}
		out = append(out, parseRecipe(body, join(dir, e.Name()), scope))
	}
...
func RecipeIndexMarkdown(recipes []Recipe) string {
	...
	b.WriteString("<!-- Generated by `make caps` from pkg/aboard/recipes/builtin/*.md. Do not edit. -->\n\n")
