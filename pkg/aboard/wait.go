...
//
// Why this exists rather than an agent polling the board document: a session that wants
...
func Wait(ctx context.Context, root Root, name, by, forWhat, note string, timeout time.Duration, out io.Writer) (int, error) {
	...
	if err := json.NewDecoder(resp.Body).Decode(&ev); err != nil {
		return ExitFailed, fmt.Errorf("unreadable answer from %s", inst.URL)
	}
	if resp.StatusCode != http.StatusOK {
		return ExitFailed, fmt.Errorf("board returned %d", resp.StatusCode)
	}
...
func Poke(ctx context.Context, root Root, name, by, note string, out io.Writer) error {
	...
	fmt.Fprintf(out, "poked %s — released %d waiting session%s\n",
