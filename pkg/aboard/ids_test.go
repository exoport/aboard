...
func TestReconcileNextID(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			incoming: `{"nextId":9,"tabs":[{"id":"ab7"}]}`,
			current:  `{"nextId":42,"tabs":[{"id":"ab7"}]}`,
			want:     42,
		},
		{
			name:     "an id above both counters raises it past that id",
			incoming: `{"nextId":3,"tabs":[{"id":"ab99"}]}`,
		...
		},
		{
			...
			current:  `{"nextId":3,"tabs":[{"id":"ab160"}]}`,
			want:     161,
		},
		{
			name:     "ids nested deep inside a tab's state count too",
			incoming: `{"nextId":2,"tabs":[{"id":"ab1","state":{"nodes":[{"id":"ab77","marks":[{"id":"ab203"}]}]}}]}`,
			current:  `{"nextId":2,"tabs":[]}`,
			want:     204,
		},
		{
			name:     "a stack block's own blocks are walked as well",
			incoming: `{"nextId":1,"tabs":[{"id":"ab1","state":{"blocks":[{"id":"ab310","state":{"rows":[{"id":"ab311"}]}}]}}]}`,
			current:  `{}`,
			want:     312,
		},
		{
		...
		},
		{
			...
			incoming: `{"nextId":5,"tabs":[{"id":"ab2","state":{"fields":[{"id":"strategy"},{"id":"window-2024"}]}}]}`,
			current:  `{"nextId":6}`,
			want:     6,
		},
		{
		...
		},
		{
			...
			incoming: `{"nextId":31,"tabs":[{"id":"ab44"}]}`,
			current:  `{not json`,
			want:     45,
		},
		{
			...
			current:  `{"nextId":12,"tabs":[{"id":"ab11","state":{"nodes":[{"id":"ab400"}]}}]}`,
			want:     401,
		},
	}
...
func TestReconcileNextIDWithNoCurrentDocument(t *testing.T) {
	if got := reconcileNextID([]byte(`{"nextId":4,"tabs":[{"id":"ab9"}]}`), nil); got != 10 {
...
// their tag, nor when the tag changed, so it is pinned rather than left to the
// table above. Both retired forms are rows here on purpose: a board written
// before 2026-08-28 is full of "bb" ids and they must keep raising the counter,
// or the first write after an upgrade hands out an id something already has.
func TestIDCounterReadsTheNumericTail(t *testing.T) {
	cases := map[any]int{
		"ab147":     147,
		"147":       147,
		"n147":      147, // the spike's tag
		"bb147":     147, // this project's, until 2026-08-28
		"strategy":  0,
		"ab":        0,
		"ab12a":     0,
		"AB12":      0, // the pattern is lower-case by design
