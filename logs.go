...
//
// The design constraint came from the human, and it was right: board.json is
// rewritten whole on every write and it is tracked in git, so an appending log
...
func (s *server) logPath(tab string) (string, bool) {
	...
	return filepath.Join(filepath.Dir(s.stateFile), instanceDir, logDirName, tab+".log"), true
}
...
func logCLI(name, tab string) error {
