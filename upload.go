...
// there is.
//
// Uploads land in uploads/ rather than assets/, and that is not arbitrary:
// assets/ is compiled into the binary with //go:embed, so a file written there at
// runtime is invisible until the next build. uploads/ is served from disk in both
// modes.
//
...
func (s *server) handleUpload(w http.ResponseWriter, r *http.Request) {
	...
	dir := filepath.Join(filepath.Dir(s.stateFile), uploadDir)
	if err := os.MkdirAll(dir, 0o755); err != nil {
		writeJSON(w, http.StatusInternalServerError, map[string]string{"error": "cannot create uploads/"})
		return
	}
...
func (s *server) serveUpload(w http.ResponseWriter, r *http.Request) {
	name := filepath.Base(strings.TrimPrefix(r.URL.Path, "/"+uploadDir+"/"))
	if name == "." || name == ".." || name == "/" || name == "" {
		http.Error(w, "not found", http.StatusNotFound)
		return
	}
	full := filepath.Join(filepath.Dir(s.stateFile), uploadDir, name)
