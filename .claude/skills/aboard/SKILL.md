...
with `aboard serve --detach`). **From an agent session, always start it detached**:
`aboard serve … &` — `nohup` or not — stays in your shell's process group, dies when
...
returns once the board answers. It answers for THIS project only; `aboard boards` is the
