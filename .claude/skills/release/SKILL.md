---
name: release
description: 'Full release workflow for aboard: pre-flight checks (clean tree, CHANGELOG, no duplicate tag) → local CI gate (make ci-local) → push main → poll push CI → final tag → poll release workflow → cosign signature verification. Use when the user says "/release", "cut a release", "tag a release", or "ship vX.Y.Z".'
...
This flow has no rc-tag pre-release gate, and that is a decision inherited rather than a gap: the sibling-tag version of it was dropped in `apex_process_ape` after rc and final annotated tags landed on the same commit and goreleaser's `git describe`-based tag resolution misrouted the build artifacts to the rc prerelease. The remote gate is the regular push-to-`main` CI run instead; the final tag goes on the same SHA, and there is no sibling rc tag to confuse goreleaser.
...
4. Capture the GitHub repo slug:
   ...
   ```
   Parse `{owner}/{repo}` from the URL (handles both HTTPS and SSH forms). Set `{repo_slug}` = `exoport/aboard` (verify against the parsed value; fail loud if they differ).
...
If `{autonomous}` is false: ask "Proceed with `make ci-local`? (this takes a couple of minutes — it ends on a goreleaser snapshot)" — wait for confirmation.
...
This is a long-running command (a couple of minutes: tests, lint, govulncheck, doc links, a Windows cross-compile and a goreleaser snapshot). Stream output. HALT if the exit code is non-zero. Message: "`make ci-local` failed. Fix the issues and re-run `/release`."
...
curl -sfL "${base_url}/aboard_checksums.txt"        -o "${tmp_dir}/aboard_checksums.txt"
curl -sfL "${base_url}/aboard_checksums.txt.bundle" -o "${tmp_dir}/aboard_checksums.txt.bundle"
```
...
Every aboard release ships a Sigstore bundle (`aboard_checksums.txt.bundle`:
cert + signature + SCT + Rekor inclusion proof), verified with
`--bundle … --new-bundle-format`. The signing block in `.goreleaser.yaml` has
been keyless-with-a-bundle since the first tag, so there is no older detached
`.sig` + `.pem` form to fall back to.

```bash
cosign verify-blob \
  --bundle "${tmp_dir}/aboard_checksums.txt.bundle" \
  --new-bundle-format \
  --certificate-identity "https://github.com/{repo_slug}/.github/workflows/release.yml@refs/tags/{version}" \
  --certificate-oidc-issuer "https://token.actions.githubusercontent.com" \
  "${tmp_dir}/aboard_checksums.txt"
```
