...
#
# Empty, and expected to stay that way for a while: aboard's whole dependency
# list is cobra + pflag + yaml.v3 and their closure, so there is very little
# surface for an advisory to land on. An entry here is a decision, not a
# convenience — write the justification with it or the next bump inherits a
# silence nobody can date.
ALLOW = set()
