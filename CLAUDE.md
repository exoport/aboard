...
- **`make smoke` fails rather than skips.** Ten sections used to print `skip …` and let the suite exit 0, so a third of the checks could be absent with nothing to say so — reachable by `make clean`, `make install` or `make dev` (no binary), or by aiming it at a board without the tab a section needs. `smoke: build` now, and every skip is a FAIL naming the section AND what it needed, which is almost always `aboard init --example` in the scratch project. No browser at all is exit 2, not a silent pass.
