...
- the note and requests strips fold into one remembered line (`localStorage`
  `aboard.context`, per viewer, global);
- a tab with no note had drawn an empty "THIS TAB IS FOR" line since the spike;
- the `decision-wizard-with-live-summary` library recipe covers per-item sign-off.

The human then asked for **`aboard shot <tab>`**, and `test/shot.sh` is gone with it.
Then for **"what does not fit"**: receipts carry `width` and `clipped`, which `aboard
rendered` prints and `aboard shot` reads back out of the same chromium run's `--dump-dom`.
The idea came from that session: it checked 98 HTML slides for overflow by measuring,
not by looking, and found four.
Two of the peer's claims were wrong and are worth knowing as the SHAPE of a mistake:
`field` "cannot grow" (it can: `grow` is in `commonProps`, which the peer read past, and
it had stripped `grow` itself while copying nodes), and a headless screenshot of a `ui`
tab "cannot be taken" (`?nosse=1&tab=` always worked; the skill only said `make shot`,
which exists nowhere but here).
...
| `pkg/aboard/shot.go`              | `aboard shot`: a picture of a tab through a headless chromium, and everything a hand-rolled screenshot got wrong, in one place. Needs a RUNNING board; reads only, and posts no mount receipt. `make shot` runs it with this checkout's binary. |
...
make shot          # LOCAL ONLY: `aboard shot` (PROJECT=<dir> SHOT_TABS="<tab> <tab>" SHOT_FLAGS="--help-panel")
...
- **Colours only from `app.css` tokens, in two variants, and a project may patch them.** 21 tokens, each declared twice — `:root` is DARK and is the default, `:root[data-theme="light"]` is light — with text pinned to WCAG AAA and no hex in any view. The palette is described by what it IS (a neutral near-black ramp, one olive accent, a periwinkle for what an agent says, an orange for what the human asks) rather than by the name of the editor theme it came from; the product name is gone from the tree. `TestBothThemesDeclareTheSameTokens` refuses a token declared in one variant and forgotten in the other, which is the drift that renders as no error at all: an un-redeclared custom property simply inherits, so a light board comes up with one black-on-black label and nothing anywhere says so. The periwinkle token is `--agent`; `claude` resolves to nothing, and a write naming an unknown colour warns.
  - **The switch is per viewer** — a topbar button, `localStorage` under `aboard.theme`, stamped on `<html>` by a classic script in the head so the first paint is already right. It never goes near the state document, like scroll, zoom and `?chrome=`. **Dark is the default for every viewer and the system colour scheme is never consulted** — the human's decision on 2026-09-13, when Moonwatcher asked for `prefers-color-scheme`. What they took instead is **`?theme=dark|light`**: a host's word for one page load, read by that same head script ahead of the stored choice and the project default, written nowhere, and overruled by pressing the switch. It exists because the `theme` message can only arrive after load, so a light host otherwise shows the board dark for a moment first. There is deliberately no keyboard shortcut: a shell-level key is taken away from every renderer that might want it.
...
- **The browser reports what it drew, into a sidecar.** After every mount — and, debounced,
  ...
  mount-only sweep would ever see it. **It also says what did not fit**, at the window
  width it had: `cut`, `spill` or `scroll`, per box, with the pixels over. Only a renderer
  with a `measure()` hook contributes: `ui` (its own components, open panel only, named
  by panel, innermost spill only), `html` (what its frame measures from inside, cut boxes
  only), and `stack` passing its blocks' reports through. The rest are left out on
  purpose. They draw the board's own layouts, which truncate by design, and a report
  that always lists them would be skipped like any warning that always fires. **A page
  `aboard shot` opens posts NO receipt**
  (`?shot=1`): `wait --for "rendered <id>"` is a session waiting for a PERSON to open the
  tab, and an agent photographing it must not release that wait or inflate the mount count.
  It writes the same sweep into its own DOM instead (`script#aboard-shot-report`), where
  `shot` reads it.
...
- **Per-viewer UI state never goes in the state file** — selection, zoom, collapsed blocks, marks-hidden, chat drafts, mount receipts, each tab's scroll offset (`sessionStorage`, `aboard.scroll.<tab>`), the open panel of a `ui` `tabs` component (`sessionStorage`, `aboard.panel.<tab>::<component>`), whether the note and requests strips are folded (`localStorage`, `aboard.context`, one setting for every tab because the preference is about screen space), and the chrome a host asks for when it frames the board (`?chrome=full|notabs|none`). Two viewers can look at one board in the same second and must disagree about all of it while agreeing about content, so it lives in the URL — [the shell's URL surface](docs/reference/http-api.md).
...
- **The browser suite cannot touch your board any more.** `make e2e` seeds a temp root, serves it in-process on a free port, and deletes it. The old `make smoke` had to be aimed at a real board with `PROJECT=`, WROTE to it, and poked the notify channel — releasing any session genuinely blocked on `aboard wait`. `make shot` still takes `PROJECT` and still needs a running server, because `aboard shot` reads the board and writes only pictures.
- **A failing `$(...)` aborts a `set -e` script with no message at all.** `sh` is `dash` here, so `BASE=$(sed … missing-file)` ended the old shell suite instantly and `make` printed nothing but its own error line — it read as "the suite is broken", not "the file is missing". No `set -e` script is left in the tree (`test/shot.sh` was the last, retired on 2026-09-22 for `aboard shot`), so this is a warning for the next one: every command substitution whose failure is survivable needs `|| true`. Same family as `$(cmd; echo $?)` reading empty.
...
- **Take pictures with `aboard shot`, never a hand-rolled chromium command**: every item below is a way a hand-rolled one came out empty or wrong, and `pkg/aboard/shot.go` knows all of them.
  - **A headless screenshot needs `?nosse=1`.** The SSE stream never closes, so chromium never reaches network-idle and writes no file at all — exit 2, no message.
  - **Headless chromium does not reliably paint iframe content**, so an `html` tab is shot at `/tab/<id>/html` directly. `--virtual-time-budget` also starves cross-process `postMessage`, which makes frame auto-sizing look broken when it is fine in a real browser.
  - **`--screenshot` draws a SCROLLED document wrongly.** Measured 2026-09-22: a 465px scroll gave a 465px blank band above the head, and a node 1600px down gave a solid black frame. A `?shot=1` page therefore hands a deep link's scroll offset to `#views` as a transform and stays at scroll 0.
  - **A fresh headless profile has no remembered tab or panel**, so the tab and `node=` go in the URL, and a `node=` the tab does not have is REFUSED — the page would open on the first panel and look exactly like success.
...
- **`apply` succeeding is not evidence that anything renders.** It prints `applied` and exits 0 for a document that draws an empty box — `ui` is the worst offender, because an unknown component shows a marker but an unknown PROP shows nothing at all. Read the stderr warnings, then `aboard shot` the tab and **look at the picture**.
- **A snap-confined chromium can write only under `$HOME`, and NOT inside a hidden directory at its top** (`~/.cache`, `~/.local`). Anywhere else it writes nothing and says "No such file or directory", which is not true. This file used to say a scratch project under `$HOME` was the way out; one under `~/.cache` is not, and that is exactly where the `TMPDIR` advice above puts scratch work. `aboard shot` renders into `~/snap/<name>/common/aboard-shots/` and moves the file to the project's `.aboard/run/shots/`, so the project can be anywhere. It clears each previous PNG first — a stale file is indistinguishable from a fresh one — and exits **1** if ANY picture was not written. `test/shot.sh` exited 0 on a partial run, which suited a person reading the terminal and not an agent reading the status.
- **`CSS` inside a view module is usually the view's stylesheet string, not the browser's `CSS` object.** Every renderer declares `` const CSS = `…` ``, so `CSS.escape(…)` in `views/ui.js` threw "CSS.escape is not a function" and took a deep link down with it. Compare dataset values instead of building a selector, or say `globalThis.CSS`.
- **An author `display:` rule beats the `hidden` attribute.** The browser's own `[hidden] { display: none }` loses to any author rule, so `.tab-note { display: flex }` made `$tabNote.hidden = true` do nothing, and every tab without a note drew an empty "THIS TAB IS FOR [Add]" line in the sticky head from the spike until 2026-09-22. Nobody saw it because every tab on the example board has a note. An element toggled by `hidden` that also has a `display` rule needs its own `[hidden] { display: none; }`.
